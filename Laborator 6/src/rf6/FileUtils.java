...
public class FileUtils 
{
	...
	{
		Neighbor[] arrNeighbor = new Neighbor[learningSet.length];
		
		for(int i=0; i<arrNeighbor.length;i++)
		{
			arrNeighbor[i] = new Neighbor();
			arrNeighbor[i].grade = Double.valueOf(learningSet[i][0]);
			arrNeighbor[i].family = learningSet[i][1];
			arrNeighbor[i].distance = Math.abs(arrNeighbor[i].grade - gradeUnknown);
		}
		
		Arrays.sort(arrNeighbor);
		...
		frequencyName.add(arrNeighbor[0].family);
		System.out.println("1 : " + arrNeighbor[0].grade + " & " + arrNeighbor[0].family);
	    for (int i = 1; i < k; i++)
	    {
			System.out.println(i+1 + " : " + arrNeighbor[i].grade + " & " + arrNeighbor[i].family);

	    	  int pos = frequencyName.indexOf(arrNeighbor[i].family);
	    	  if(pos == -1)
	    	  {
	    			frequencyNumber.add(1);
	    			frequencyName.add(arrNeighbor[i].family);
	    	  }
	    	  else
	    	  {
	    ...
	    for(int i=0; i<frequencyNumber.size();i++)
	    	if (max < frequencyNumber.get(i))
	    		{max = frequencyNumber.get(i); index = i;}
